...
# class SavePointEncoderCallback(TrainerCallback):
#     def on_save(
#         self,
...
#         **kwargs,
#     ):
#         # kwargs['model'] 是当前的 PeftModel 实例 (get_peft_model 的结果)
#         current_model = kwargs['model']
#         output_dir = args.output_dir # 获取 Trainer 的输出目录
#         checkpoint_dir = os.path.join(output_dir, f"checkpoint-{state.global_step}")

#         # 确保 checkpoint 目录存在 (Trainer 会创建它，但以防万一)
#         if not os.path.exists(checkpoint_dir):
#             os.makedirs(checkpoint_dir)

#         # 构造 point_encoder 权重的保存路径
#         point_encoder_save_path = os.path.join(checkpoint_dir, "point_encoder_weights.bin")

#         # 访问 point_encoder 模块并保存其状态字典
#         # path: current_model -> PeftModel -> base_model -> CADRecode -> point_encoder
#         # 注意: current_model.base_model 是 PeftModel 内部用来包装原始模型的部分
#         # current_model.base_model.model 才是 CADRecode 实例
#         torch.save(current_model.base_model.model.point_encoder.state_dict(), point_encoder_save_path)
#         print(f"[{state.global_step}] Saved point_encoder weights to: {point_encoder_save_path}")

#         # 返回 control，告诉 Trainer 是否继续正常流程
#         return control
...
class SavePointEncoderCallback(TrainerCallback):
    def on_save(
        ...
        **kwargs,
        ...
        # path: current_model -> PeftModel -> base_model -> CADRecode -> point_encoder
        # current_model.base_model.model 才是 CADRecode 实例
        point_encoder_module = current_model.base_model.model.point_encoder

        # --- 核心修改部分 ---
        # 创建一个新的 state_dict 来存储要保存的 point_encoder 权重
        # 目标是让其键名与标准的 FourierPointEncoder (内部是 nn.Linear) 结构匹配
        point_encoder_state_to_save = {}
        for name, param in point_encoder_module.named_parameters():
            # 检查参数名是否包含 LoRA 特有的 'base_layer' 或 'lora_A/B'
            if "projection.base_layer." in name:
                # 剥离 'base_layer.' 前缀，使其与非 LoRA 化的 Linear 层键名匹配
                new_name = name.replace("projection.base_layer.", "projection.")
                point_encoder_state_to_save[new_name] = param.clone().detach().cpu()
            elif "projection.lora_A" in name or "projection.lora_B" in name:
                # 忽略 LoRA 适配器本身的权重，我们只关心原始的线性层权重
                # 如果你想保存 LoRA 适配器本身，可以根据需要调整逻辑，但通常这里只需要原始权重。
                pass
            else:
                # 对于 point_encoder 中其他可能存在的非 LoRA 化的参数，直接添加
                # （例如，如果 FourierPointEncoder 还有其他层没有被 LoRA 化）
                point_encoder_state_to_save[name] = param.clone().detach().cpu()

        torch.save(point_encoder_state_to_save, point_encoder_save_path)
        print(f"[{state.global_step}] Saved point_encoder weights to: {point_encoder_save_path}")
...
if __name__ == "__main__":
    ...
    trainer = SFTTrainer(
        ...
        callbacks=[SavePointEncoderCallback()],
    )

    print("Starting training...")
    # trainer.train()
    trainer.train(resume_from_checkpoint="/home/user2/wns/cad-recode/cad-recode-qwen3/checkpoint-65200")
